...
export default function Home() {
  ...
  const initBookList = async (isRestart: boolean = false) => {
    setIsLoading(true);
    ...
    if (res?.data.data.books) {
      setIsLoading(false);
      if (isRestart) {
        setBooks((prev) => res?.data.data.books);
      } else {
  ...
  useEffect(() => {
    ...
    initTagList();
    initWriterList();
  }, [page, initBookList, initLatestPoint, initTagList, initWriterList]);
  ...
  return (
    <AdminLayout>
      ...
      <Stack flexDirection="row" flexWrap="wrap" gap={2}>
        {books?.map((item, key) => (
          <BookCard key={key} item={item} purchase={purchase} />
        ))}
